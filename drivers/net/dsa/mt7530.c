...
static int mt7530_sa_state_set(struct dsa_switch *ds, int port, bool offlearning)
{
	if (NULL == ds || port < 0 || port >= MT7530_NUM_PORTS)
		return -1;

	if (offlearning)
	{
		mt7530_set(ds->priv, MT7530_PSC_P(port), 0x10);
	}
	else
	{
		mt7530_clear(ds->priv, MT7530_PSC_P(port), 0x10);
	}

	return 0;
}

static int mt7530_force_fwd(struct dsa_switch *ds, int port)
{
	struct mt7530_priv *priv = ds->priv;
	u32 port_bitmap = BIT(MT7530_CPU_PORT);
	int i;
...
#ifdef CONFIG_TP_IMAGE
	.port_sa_state_set = mt7530_sa_state_set,
	.port_set_force_fwd_map =  mt7530_force_fwd,
#endif
};
