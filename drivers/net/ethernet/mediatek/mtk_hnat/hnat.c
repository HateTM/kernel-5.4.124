...
struct hnat_accounting *tp_collect_acct[MAX_PPE_NUM];
#endif

//#ifndef CONFIG_TP_IMAGE /*开启内核roaming检测*/
#if 1
...
#define SKIP_CT_NUM (66) // 64 clinet + 2 guest

static int ct_cache_init(struct ct_mark_tbl * ct_skip)
{
	ct_skip->mem = kzalloc((SKIP_CT_NUM + 1)*sizeof(struct ct_mark_entry), GFP_KERNEL);
	if (!ct_skip->mem)
	{
		return -ENOMEM;
	}

	ct_skip->node = PTR_ALIGN(ct_skip->mem, sizeof(struct ct_mark_entry));

	return 0;
}

static void ct_cache_destroy(struct ct_mark_tbl * ct_skip)
{
	ct_skip->node = NULL;
...
inline struct ct_mark_entry * ct_mark_match(struct ct_mark_tbl *tbl, u32 mark)
{
	struct ct_mark_entry * entry = tbl->node;
	int i;
	...
	for (i = 0 ; (i < SKIP_CT_NUM) && entry[i].mark; i++)
	{
		if ((entry[i].mark & entry[i].mask) == (mark & entry[i].mask))
		{
			return &entry[i];
		}
	}
...
void ct_mark_insert(struct ct_mark_tbl *tbl, u32 mark, u32 mask, long group)
{
	int i;
	struct ct_mark_entry * entry = tbl->node;
	...
	if (NULL == entry)
	{
		pr_info("null ct entry!");
		return;
	}

	for (i = 0; (i < SKIP_CT_NUM) && entry[i].mark; i++)
	{
		if ((entry[i].mark == mark) && (entry[i].mask == mask))
		{
			pr_info("ct entry exist!");
			return;
		}
	}
	...
	{
		pr_info("ct entry exceed!");
	}

	return;
}

void ct_mark_del_by_mark(struct ct_mark_tbl *tbl, u32 mark, u32 mask)
{
	struct ct_mark_entry * entry = tbl->node;
	int i, del = -1;
	...
	for (i = 0 ; (i < SKIP_CT_NUM); i++)
	{
		...
		if ((del < 0) && (entry[i].mark == mark) && (entry[i].mask == mask))
		{
			del = i;
		}
	}
	...
	if (del >= 0)
	{
		if (i != (del+1))
		{
			memmove((void *)&entry[del], (void *)&entry[i-1], sizeof(struct ct_mark_entry));
		}

		memset((void *)&entry[i-1], 0, sizeof(struct ct_mark_entry));
	}

	return;
}

void ct_mark_del_by_group(struct ct_mark_tbl *tbl, long group)
{
	struct ct_mark_entry * entry = tbl->node;
	int i, tail;
	...
	for (tail = (SKIP_CT_NUM - 1); tail >= 0; tail--)
	{
		if (0 != entry[tail].mark)
		{
			break;
		}
	}
	...
	{
		if (group == entry[i].group)
		{
			if (i != tail)
			{
				memmove((void *)&entry[i], (void *)&entry[tail], sizeof(struct ct_mark_entry));
			}
			memset((void *)&entry[tail], 0, sizeof(struct ct_mark_entry));
			tail = tail - 1;
			// i = i;
		}
		else
...
void ct_mark_clear(struct ct_mark_tbl *tbl, long group)
{
	if (tbl->node)
	{
		if (-1 == group)
		{
			memset(tbl->node, 0, SKIP_CT_NUM*sizeof(struct ct_mark_entry));
		}
		else
		{
			ct_mark_del_by_group(tbl, group);
		}
	}
...
void ct_mark_dump(struct ct_mark_tbl *tbl)
{
	int i;
	struct ct_mark_entry * entry = tbl->node;
	...
	for (i = 0 ; (i < SKIP_CT_NUM) && entry[i].mark; i++)
	{
		pr_info("%x-%x-%ld\n", entry[i].mark, entry[i].mask, entry[i].group);
	}
}
#endif
...
static int hnat_start(int ppe_id)
{
	...
	if (hnat_priv->data->per_flow_accounting) {
		...
		if (!hnat_priv->acct[ppe_id])
			return -1;
#if defined(CONFIG_TP_MTKHNAT_STATS)
		tp_collect_acct[ppe_id] =
			kzalloc(hnat_priv->foe_etry_num * sizeof(struct hnat_accounting),
				GFP_KERNEL);
		if (!tp_collect_acct[ppe_id])
			return -1;
#endif
	}
	...
	/* ext data init */
	hnat_priv->ext_data.foe_table_cpu_ext[ppe_id] = 
		kzalloc(hnat_priv->foe_etry_num * sizeof(struct foe_entry_ext), 
			GFP_KERNEL);
	if (!hnat_priv->ext_data.foe_table_cpu_ext[ppe_id])
		return -1;
...
static void hnat_stop(int ppe_id)
{
	...
	if (hnat_priv->ext_data.foe_table_cpu_ext[ppe_id])
	{
		kfree(hnat_priv->ext_data.foe_table_cpu_ext[ppe_id]);
	}
#endif
}
...
#if defined(CONFIG_NET_MEDIATEK_HW_QOS)
	memset(&hnat_priv->ext_data.hqos_settings, 0, sizeof(struct hnat_qos_settings));
#endif
...
#if defined(CONFIG_TP_MTKHNAT_CT_SKIP) && defined(CONFIG_TP_IMAGE)
	err = ct_cache_init(&hnat_priv->ct_skip);
	if (err)
		return err;
#endif
...
#if defined(CONFIG_TP_MTKHNAT_CT_SKIP) && defined(CONFIG_TP_IMAGE)
	ct_cache_destroy(&hnat_priv->ct_skip);
#endif
	return 0;
}
