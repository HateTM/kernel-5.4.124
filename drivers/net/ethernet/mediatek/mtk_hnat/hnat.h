...
struct foe_entry_ext {
	u8 ori_smac[ETH_ALEN];
...
#ifndef MTK_QDMA_TX_NUM
#define MTK_QDMA_TX_NUM 16
#endif

struct hnat_qos_settings {
	bool enable;
	int queue_mark[MTK_QDMA_TX_NUM];
};

#define HNAT_QOS_ENABLE() (hnat_priv->ext_data.hqos_settings.enable)

#endif

struct mtk_hnat_ext_data {
	struct foe_entry_ext 		*foe_table_cpu_ext[MAX_PPE_NUM];
#if defined(CONFIG_NET_MEDIATEK_HW_QOS)
	struct hnat_qos_settings 	hqos_settings;
#endif
	int ipv4_homesheild_started;

};

#if defined(CONFIG_TP_MTKHNAT_CT_SKIP)
struct ct_mark_entry {
	u32  mark;
	u32  mask;
	long group;
};
...
#if defined(CONFIG_TP_IMAGE)
	struct mtk_hnat_ext_data ext_data;

#if defined(CONFIG_TP_MTKHNAT_CT_SKIP)
	struct ct_mark_tbl ct_skip;
#endif
#endif
...
/*PPE_GLO_CFG*/
#define BIT_L4F_DRP_EN BIT(2)
#define BIT_IPV4F_DRP_EN BIT(3)
#define SP_CMP_EN BIT(25)
...
#define IS_IPV4_GRP_DSLITE_MAPE(x) (IS_IPV4_HNAPT(x) | IS_IPV4_HNAT(x) | IS_IPV4_DSLITE(x) | IS_IPV4_MAPE(x))
...
#if defined(CONFIG_TP_MTKHNAT_STATS)
extern struct hnat_accounting *tp_collect_acct[MAX_PPE_NUM];
#endif
