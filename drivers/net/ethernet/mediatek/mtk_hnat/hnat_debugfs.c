...
uint32_t foe_dump_pkt(struct sk_buff *skb)
{
	...
	if (IS_IPV4_GRP(entry)) {
		...
		pr_info("State = %s, proto = %s\n", entry->bfib1.state == 0 ?
			...
			"UDP" : "Unknown");
		pr_info("qid = %d, fqos = %d\n", entry->ipv4_hnapt.iblk2.qid, entry->ipv4_hnapt.iblk2.fqos);
...
#if defined(CONFIG_TP_MTKHNAT_STATS)
	pr_info("              6                foe_clear_all_bind_entries\n");
	pr_info("              7    <MAC>       foe_entry_delete_by_mac\n");
#endif
	return 0;
}
...
struct hnat_accounting *hnat_get_count(struct mtk_hnat *h,
...
#if defined(CONFIG_TP_MTKHNAT_STATS)
#define PRINT_EXTINFO(m, src_mac,dst_mac,in,out) {if (src_mac != NULL && dst_mac != NULL && in != NULL && out != NULL) \
	seq_printf(m, " |SMAC=%pM=>DMAC=%pM|IN=%s=>OUT=%s\n", \
		   src_mac, dst_mac, in, out); }
#endif
...
void dbg_dump_entry(struct seq_file *m, struct foe_entry *entry,
		    int ppe_id, uint32_t index)
{
	__be32 saddr, daddr, nsaddr, ndaddr;

	struct foe_entry_ext *ext_entry = NULL;
	unsigned char ori_mac[ETH_ALEN];
	unsigned char in_ifname[16] = {0};
	unsigned char out_ifname[16] = {0};
	unsigned char h_dest[ETH_ALEN];
	struct hnat_accounting *acount = NULL;
	...
	ext_entry = &hnat_priv->ext_data.foe_table_cpu_ext[ppe_id][index];
	if (ext_entry != NULL &&  ext_entry->ori_smac != NULL)
	{
		memcpy(ori_mac, ext_entry->ori_smac, ETH_ALEN);
		memcpy(in_ifname, ext_entry->ifname, 16);
		memcpy(out_ifname, ext_entry->out_ifname, 16);
	}
	/*dont get hnat account ,let tp collect old & new account info correctly*/
	...
	if (IS_IPV4_HNAPT(entry)) {
		...
		PRINT_COUNT(m, acount);
		seq_printf(m,
			   "NAPT(%d): %pI4:%d->%pI4:%d => %pI4:%d->%pI4:%d,qid=%d,fqos=%d",
			   ...
			   entry->ipv4_hnapt.iblk2.qid,
			   entry->ipv4_hnapt.iblk2.fqos);
		PRINT_EXTINFO(m, ori_mac, h_dest, in_ifname, out_ifname);
	} else if (IS_IPV4_HNAT(entry)) {
			*((u32 *)h_dest) = swab32(entry->ipv4_hnapt.dmac_hi);
			*((u16 *)&h_dest[4]) =
				swab16(entry->ipv4_hnapt.dmac_lo);
	...
	if (IS_IPV4_DSLITE(entry)) {
		...
		*((u16 *)&h_dest[4]) =
		...
		PRINT_COUNT(m, acount);
		seq_printf(m,
			   "IPv6_5T(%d): %08X:%08X:%08X:%08X:%d => %08X:%08X:%08X:%08X:%d,qid=%d,fqos=%d",
...
int __hnat_entry_read(struct seq_file *m, void *private, int ppe_id)
{
	...
	while (entry < end) {
		if (entry->bfib1.state == dbg_entry_state) {
			cnt++;
#if defined(CONFIG_TP_MTKHNAT_STATS)
			dbg_dump_entry(m, entry, ppe_id, hash_index);
#else
...
#if defined(CONFIG_TP_MTKHNAT_STATS)
	case 6:
		printk(" foe_clear_all_bind_entries entry state BIND->INVALID\n");
		foe_clear_all_bind_entries(NULL);
		return len;
	case 7:
		/*delete mac entry change state BIND->INVALID*/
		p_token = strsep(&p_buf, p_delimiter);
		if (p_token)
			pr_info("apsd delete mac is %s \n", p_token);
		/*to do check mac format*/

		foe_entry_delete_by_mac(p_token);
		return len;
#endif
	default:
		pr_info("no handler defined for command id(0x%08lx)\n\r", arg0);
				...
				size_t length, loff_t *offset)
{
	...
	if ((exp == 4 && rate > 100) || exp > 4)
		qdma_tx_sch &= ~(BIT(11) << shift);
				...
				size_t length, loff_t *offset)
{
	...
	qtx_sch |= max_exp & 0xf;
	if ((max_exp == 4 && max_rate > 100) || max_exp > 4)
		qtx_sch &= ~BIT(11);
...
#if defined(CONFIG_TP_MTKHNAT_CT_SKIP) && defined(CONFIG_TP_IMAGE)
static ssize_t hnat_skip_ct_write(struct file *file, const char __user *buffer,
				 size_t count, loff_t *data)
{
	char buf[64];
	u32 mark, mask;
	...
	if (count > 0)
	{
		if (copy_from_user(buf, buffer, count > 64 ? 64 : count))
			return -EFAULT;
		
		if (0 == strncmp(buf, "clear", 5))
		{
			sscanf(buf, "clear:%ld", &group);
			ct_mark_clear(&hnat_priv->ct_skip, group);
		}
		else if (2 <= sscanf(buf, "add:%x %x %ld", &mark, &mask, &group))
		{
			ct_mark_insert(&hnat_priv->ct_skip, mark, mask, group);
			foe_clear_all_bind_entries(NULL);
		}
		else if (2 == sscanf(buf, "del:%x %x", &mark, &mask))
		{
			ct_mark_del_by_mark(&hnat_priv->ct_skip, mark, mask);
		}
	}
...
static int hnat_skip_show(struct seq_file *m, void *private)
{
	ct_mark_dump(&hnat_priv->ct_skip);

	return 0;
}

static int hnat_skip_ct_open(struct inode *inode, struct file *file)
{
	return single_open(file, hnat_skip_show, file->private_data);
}

static const struct file_operations hnat_skip_ct_fops = {
	.open = hnat_skip_ct_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = hnat_skip_ct_write,
	.release = single_release,
};
#endif
				      ...
				      size_t count, loff_t *data)
{
	...
	for (i = 0; i < CFG_PPE_NUM; i++) {
		...
		glo_cfg = readl(hnat_priv->ppe_base[i] + PPE_GLO_CFG);

		if (mape_toggle) {
			ppe_cfg &= ~BIT_IPV4_DSL_EN;
			ppe_cfg &= ~BIT_IPV4_NAT_FRAG_EN;
			glo_cfg &= ~BIT_L4F_DRP_EN;
			glo_cfg &= ~BIT_IPV4F_DRP_EN;
			glo_cfg |= SP_CMP_EN;
		} else {
			ppe_cfg |= BIT_IPV4_DSL_EN;
			ppe_cfg |= BIT_IPV4_NAT_FRAG_EN;
		...
		writel(glo_cfg, hnat_priv->ppe_base[i] + PPE_GLO_CFG);
	}
...
static void hnat_qos_disable(void)
{
	...
	for (id = 0; id < MAX_PPPQ_PORT_NUM; id++) {
		...
		writel((4 << QTX_CFG_HW_RESV_CNT_OFFSET) |
		       (4 << QTX_CFG_SW_RESV_CNT_OFFSET), h->fe_base + QTX_CFG(id % NUM_OF_Q_PER_PAGE));
	}
				     ...
				     size_t count, loff_t *data)
{
	char buf[8];
	int len = count;
	/*g_qos_toggle is filled in. Fix Bug 660149.*/
	extern unsigned int * g_qos_toggle;
...
static ssize_t hnat_mark_offset_show(struct file *file, char __user *user_buf,
			       size_t count, loff_t *ppos)
{
	char *buf;
	unsigned int len = 0, buf_len = 1500;
...
static ssize_t hnat_mark_offset_write(struct file *file, const char __user *buf,
				size_t length, loff_t *offset)
{
	char line[64] = {0};
	size_t size;
	...
	if (sscanf(line, "%d", &mark_offset) != 1)
		return -EFAULT;

	line[length] = '\0';

	if (mark_offset < 0 || mark_offset > 31) {
		pr_info("Invalid mark_offset.\n");
		return -EINVAL;
	}

	hnat_priv->qid_mark_offset = mark_offset;
...
static const struct file_operations hnat_mark_offset_fops = {
	.open = simple_open,
	.read = hnat_mark_offset_show,
	.write = hnat_mark_offset_write,
	.llseek = default_llseek,
};
...
static int hnat_homesheild_read(struct seq_file *m, void *private)
{
	pr_info("ipv4_homesheild_started is %d\n", hnat_priv->ext_data.ipv4_homesheild_started);
	return 0;
}

static int hnat_homesheild_open(struct inode *inode, struct file *file)
{
	return single_open(file, hnat_homesheild_read, file->private_data);
}

static ssize_t hnat_homesheild_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char buf[32];
	char *p_buf;
	char *p_token;
	...
	if((len >= 32) || copy_from_user(buf, buffer, len))
	{
		return -EFAULT;
	}
	...
	p_token = strsep(&p_buf, delim);
	if (NULL == p_token)
	{
		pr_info("ERROR1: Unknow command line: %s\n", p_buf);
		return len;
	}

	if (NULL == p_buf)
	{
		pr_info("ERROR2: Unknow command line: %s\n", p_token);
		return len;
	}

	if (0 != (cvt_res[0] = kstrtol(p_token, 10, &argv[0])) ||
		0 != (cvt_res[1] = kstrtol(p_buf, 10, &argv[1])))
	{
		pr_info("ERROR3: Unknow command line: %s %s\n", p_token, p_buf);
		return len;
	}
	...
	case 0:
		if (1 == argv[1]) {
			if (1 != hnat_priv->ext_data.ipv4_homesheild_started) {
				pr_info("hnat ipv4 homesheild is going to start\n");
				hnat_priv->ext_data.ipv4_homesheild_started = 1;
			}
			break;
		}
		else if ( 0 == argv[1]) {
			if (0 != hnat_priv->ext_data.ipv4_homesheild_started) {
				pr_info("hnat ipv4 homesheild is going to stop\n");
				hnat_priv->ext_data.ipv4_homesheild_started = 0;
			}
			break;
		}
		pr_info("ERROR4: no handler defined for command id(0x%08lx)\n", argv[1]);
		break;
	/*for further use，such as ipv6*/
	case 1:
	default:
		pr_info("ERROR5: no handler defined for command id(0x%08lx)\n", argv[0]);
	}

	return len;
}

static const struct file_operations hnat_homesheild_fops = {
	.open = hnat_homesheild_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = hnat_homesheild_write,
	.release = single_release,
};

#if defined(CONFIG_NET_MEDIATEK_HW_QOS)

static int hnat_qos_settings_read(struct seq_file *m, void *private)
{
	int queue_idx = 0;
	int mark = 0;

	pr_info("qos enable = %d\n", hnat_priv->ext_data.hqos_settings.enable);

	for (queue_idx = 0; queue_idx < MTK_QDMA_TX_NUM; queue_idx++)
	{
		mark = hnat_priv->ext_data.hqos_settings.queue_mark[queue_idx];
		if (mark)
		{
			pr_info("qdma_txq%d: mark = 0x%x\n", queue_idx, mark);
		}
	}
...
static int hnat_qos_settings_open(struct inode *inode, struct file *file)
{
	return single_open(file, hnat_qos_settings_read, file->private_data);
}

static int hnat_qos_setting_write_mark(char *p_buf)
{
	char *p_token;
	long argv[2];
	...
	if (NULL == p_token || NULL == p_buf)
	{
		pr_info("Unknow parameter 2: %s\n", (NULL != p_token) ? p_token : p_buf);
		return -1;
	}

	if (0 != kstrtol(p_token, 10, &argv[0]))
	{
		pr_info("Unknow parameter 2: %s\n", p_token);
		return -1;
	}

	if (0 != kstrtol(p_buf, 16, &argv[1]))
	{
		pr_info("Unknow parameter 3: %s\n", p_buf);
		return -1;
	}

	if (argv[0] < 0 || argv[0] >= MTK_QDMA_TX_NUM)
	{
		pr_info("queue id %ld is not exist\n", argv[0]);
		return -1;
	}
	
	hnat_priv->ext_data.hqos_settings.queue_mark[argv[0]] = argv[1];
	
	pr_info("qdma_txq%ld: mark = 0x%lx\n", argv[0], argv[1]);

	return 0;
}

static ssize_t hnat_qos_settings_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	char buf[32];
	char *p_buf;
	char *p_token;
	...
	if (NULL == p_token || NULL == p_buf)
	{
		pr_info("Unknow parameter : %s\n", (NULL != p_token) ? p_token : p_buf);
		return len;
	}

	if (0 != kstrtol(p_token, 10, &argv[0]))
	{
		pr_info("Unknow parameter 1: %s\n", p_token);
		return len;
	}
	...
	{
	case 0:
		if (0 == kstrtol(p_buf, 10, &argv[1]))
		{
			if (1 == argv[1])
			{
				if (1 != hnat_priv->ext_data.hqos_settings.enable)
				{ 
					pr_info("HQOS is going to start\n");
					hnat_priv->ext_data.hqos_settings.enable = 1;
				}
				break;
			}
			if (0 == argv[1])
			{
				if (0 != hnat_priv->ext_data.hqos_settings.enable)
				{
					pr_info("HQOS is going to stop\n");
					hnat_priv->ext_data.hqos_settings.enable = 0;
				}
				break;
			}
		}
		...
		break;
	case 1:
		hnat_qos_setting_write_mark(p_buf);
		break;
	default:
		pr_info("Unknow parameter 1: %s\n", p_token);
	}
...
static const struct file_operations hnat_qos_settings_fops = {
	.open = hnat_qos_settings_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = hnat_qos_settings_write,
	.release = single_release,
};
#endif

#if defined(CONFIG_TP_MTKHNAT_STATS) //for easymesh

#define DEVNAME_FE_5G "apclix0"
#define DEVNAME_FE_2G "apcli0"

struct tp_cliHwStat_t
{
	uint8_t cli_mac[ETH_ALEN];
	uint32_t num_packets;
	uint64_t num_bytes;
	uint8_t hw_stat_type;
};

typedef int (* TP_HYFI_FLWSTAT_CB)(struct tp_cliHwStat_t *stat_cb);
static TP_HYFI_FLWSTAT_CB tp_hyfi_flwstat_cb = NULL;

int tp_collect_hw_entry(struct foe_entry *entry,
		    int ppe_id, uint32_t index)
{
	struct foe_entry_ext *ext_entry = NULL;
	unsigned char ori_mac[ETH_ALEN];
	...
	unsigned char h_dest[ETH_ALEN];
	struct hnat_accounting *acount;
	struct hnat_accounting *old_acct;

	struct tp_cliHwStat_t clihwstat;

	memset(&clihwstat, 0 ,sizeof(struct tp_cliHwStat_t));

	if(!tp_hyfi_flwstat_cb){
		printk("debug tp_hyfi_flwstat_cb is NULL !!!\n");
		return 0;
	}
	...
	if (IS_IPV4_GRP(entry)) {
		...
		*((u16 *)&h_dest[4]) = swab16(entry->ipv4_hnapt.dmac_lo);
#if defined(CONFIG_MEDIATEK_NETSYS_V2)
	}else if (IS_IPV4_DSLITE(entry) || IS_IPV4_MAPE(entry)) {
		*((u32 *)h_dest) = swab32(entry->ipv4_dslite.dmac_hi);
		*((u16 *)&h_dest[4]) = swab16(entry->ipv4_dslite.dmac_lo);
#endif
	} else{
		*((u32 *)h_dest) = swab32(entry->ipv6_5t_route.dmac_hi);
		*((u16 *)&h_dest[4]) = swab16(entry->ipv6_5t_route.dmac_lo);
	...
	acount = &(hnat_priv->acct[ppe_id][index]);
	old_acct = &(tp_collect_acct[ppe_id][index]);

	if(!strncmp(in_ifname, DEVNAME_FE_5G, strlen(DEVNAME_FE_5G)) || !strncmp(in_ifname, DEVNAME_FE_2G,strlen(DEVNAME_FE_2G)))
	{
		memcpy(clihwstat.cli_mac, h_dest, ETH_ALEN);
		clihwstat.num_bytes = acount->bytes - old_acct->bytes;
		clihwstat.num_packets = acount->packets - old_acct->packets;
		if(debug_level >= 2)
		{
			printk("index(%d) %s=>%s %pM=>%pM [bytes:increase=%lld(%lld->%lld) packets:increase=%d(%lld->%lld)]\n",
				index, in_ifname,out_ifname, ori_mac,clihwstat.cli_mac,
				clihwstat.num_bytes, old_acct->bytes,acount->bytes,clihwstat.num_packets,old_acct->packets,acount->packets);
		}
	}

	if(!strncmp(out_ifname, DEVNAME_FE_5G, strlen(DEVNAME_FE_5G)) || !strncmp(out_ifname, DEVNAME_FE_2G, strlen(DEVNAME_FE_2G)))
	{
		memcpy(clihwstat.cli_mac, ori_mac, ETH_ALEN);
		clihwstat.num_bytes = acount->bytes - old_acct->bytes;
		clihwstat.num_packets = acount->packets - old_acct->packets;
		if(debug_level >= 2)
		{
	...
	old_acct->bytes = acount->bytes;
	old_acct->packets = acount->packets;

	if(tp_hyfi_flwstat_cb)
		tp_hyfi_flwstat_cb(&clihwstat);

	return 0;

}

void tp_hyfi_reg_flwstat_cb(TP_HYFI_FLWSTAT_CB stat_cb)
{
	tp_hyfi_flwstat_cb = stat_cb;
}
EXPORT_SYMBOL(tp_hyfi_reg_flwstat_cb);

int tp_hyfi_get_fc_flwstats(void)
{
	...
	for (i = 0; i < CFG_PPE_NUM; i++) {
		...
		for (index = 0; index < hnat_priv->foe_etry_num; entry++, index++) {
			if(entry->bfib1.state == BIND) {
				tp_collect_hw_entry(entry, i, index);
			}
		}
	}
...
EXPORT_SYMBOL(tp_hyfi_get_fc_flwstats);

#endif //CONFIG_TP_MTKHNAT_STATS
...
int hnat_init_debugfs(struct mtk_hnat *h)
{
...
#ifdef CONFIG_TP_IMAGE
	debugfs_create_file("hnat_homesheild", S_IRUGO | S_IRUGO, root, h, 
				&hnat_homesheild_fops);

#if defined(CONFIG_TP_MTKHNAT_CT_SKIP)
	debugfs_create_file("skip_ct", S_IRUGO | S_IRUGO, root, h,
			    &hnat_skip_ct_fops);
#endif

#if defined(CONFIG_NET_MEDIATEK_HW_QOS)
	debugfs_create_file("hnat_qos_settings", S_IRUGO | S_IRUGO, root, h,
				&hnat_qos_settings_fops);
#endif
	...
	debugfs_create_file("qid_mark_offset", S_IRUGO, root, h,
				&hnat_mark_offset_fops);
