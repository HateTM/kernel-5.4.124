...
static void get_ori_info(struct sk_buff *skb,const struct net_device* input)
{
	struct ethhdr *eth;
	struct foe_entry_ext *ext_entry = NULL;

	if(!skb_mac_header_was_set(skb) ||
		unlikely(!skb_hnat_is_hashed(skb)))
	{
		return;
	}
	...
	eth = eth_hdr(skb);
	if(eth != NULL)
		ext_entry = &hnat_priv->ext_data.foe_table_cpu_ext[skb_hnat_ppe(skb)][skb_hnat_entry(skb)];

	if(ext_entry != NULL){
		memcpy(ext_entry->ori_smac, eth->h_source, ETH_ALEN);
		memcpy(ext_entry->ifname,input->name,16);
	}

}
#endif /*end CONFIG_TP_MTKHNAT_STATS*/

static void get_ori_mac(struct sk_buff *skb)
{
	struct ethhdr *eth;
	struct foe_entry_ext *ext_entry;
...
extern int tp_stats_update(int inst, int dir, u_int32_t ip, unsigned char *mac, 
							u_int32_t newPpackets, u_int64_t newBytes, int istcp);

static void mtk_hnat_flow_acct_stats(u64 recent_pkts, u64 recent_bytes, struct sk_buff *skb, int dir)
{
	...
	if(!IS_IPV4_GRP_DSLITE_MAPE(entry))
	{
		return;
	}
	...
	{
		lanIp = entry->ipv4_hnapt.new_dip;
		*((u32 *)lanMac) = swab32(entry->ipv4_hnapt.dmac_hi);
		*((u16 *)&lanMac[4]) = swab16(entry->ipv4_hnapt.dmac_lo);
	}
	/* up steam */
	else
	{
		lanIp = entry->ipv4_hnapt.sip;
		memcpy(lanMac, ext_entry->ori_smac, ETH_ALEN);
	}

	istcp = !entry->ipv4_hnapt.bfib1.udp;

	tp_stats_update(0, dir, lanIp, lanMac, recent_pkts, recent_bytes, istcp);
}
#endif /* CONFIG_TP_MTKHNAT_STATS */

static void mtk_hnat_flow_acct_ct(u64 recent_pkts, u64 recent_bytes, struct nf_conn *ct, int dir)
{
	struct nf_conn_acct *ct_acct = nf_conn_acct_find(ct);
	...
	ct_acct->counter[dir].packets.counter += recent_pkts;
	ct_acct->counter[dir].bytes.counter   += recent_bytes;
}

static void mtk_hnat_flow_acct(struct sk_buff *skb, const struct net_device * out)
{
	struct nf_conn *ct;
	...
	struct hnat_accounting old_acct = {0};
	struct hnat_accounting *p_new_acct = NULL;
	...
	ppe_id = skb_hnat_ppe(skb);
	entry_idx = skb_hnat_entry(skb);

	if (entry_idx >= hnat_priv->foe_etry_num)
	{
		printk("Error: hwnat entry_idx reached limited\n");
		return;
	}
	...
	old_acct = hnat_priv->acct[ppe_id][entry_idx];

	p_new_acct = hnat_get_count(hnat_priv, ppe_id, entry_idx);
	...
	recent_pkts = p_new_acct->packets - old_acct.packets;
	recent_bytes = p_new_acct->bytes - old_acct.bytes;

	ext_entry = &hnat_priv->ext_data.foe_table_cpu_ext[ppe_id][entry_idx];

	if (entry != NULL && ext_entry != NULL )
	{
		if(out != NULL && out->name != NULL)
		{
			//pr_info("the out dev is to %s !\n",out->name);
			/*entry may be reused ,so we clear it each time*/
			memset(ext_entry->out_ifname, 0, 16);
			memcpy(ext_entry->out_ifname, out->name, strlen(out->name));
		}
	}

	if(ct && recent_bytes)
	{
		mtk_hnat_flow_acct_ct(recent_pkts, recent_bytes, ct, dir);
		mtk_hnat_flow_acct_stats(recent_pkts, recent_bytes, skb, dir);
	}
}

#if defined(CONFIG_NET_MEDIATEK_HW_QOS)
static int hnat_qos_get_qid(struct sk_buff *skb)
{
	int qid = 0;
	...
	for (qid = 0; qid < MTK_QDMA_TX_NUM; qid++)
	{
		if (hnat_priv->ext_data.hqos_settings.queue_mark[qid] == skb->mark)
		{
			break;
		}
	}

	if (qid >= MTK_QDMA_TX_NUM)
	{
		qid = 0;
	}
...
#endif /* CONFIG_NET_MEDIATEK_HW_QOS */
...
void foe_entry_delete_by_mac(char *strmac)
{
	...
	char del_mac[ETH_ALEN] = {0};

	if(!strmac)
	return;

	if(!str2mac(strmac, &del_mac[0]))
		return;

	for (i = 0; i < CFG_PPE_NUM; i++) {
		for (hash_index = 0; hash_index < hnat_priv->foe_etry_num; hash_index++) {
			entry = hnat_priv->foe_table_cpu[i] + hash_index;
			ext_entry = &hnat_priv->ext_data.foe_table_cpu_ext[i][hash_index];
			...
			if (entry->bfib1.state == BIND ) {
				...
				if (memcmp(h_dest, del_mac, ETH_ALEN) == 0 || 
						memcmp(ori_mac, del_mac, ETH_ALEN) == 0) {
					/*memset entry*/
					memset(entry, 0, sizeof(struct foe_entry));
					memset(ext_entry, 0, sizeof(struct foe_entry_ext));
					hnat_cache_ebl(1);
					pr_info("foe_entry_delete_by_mac[index(%d), SMAC= %pM =>DMAC= %pM]\n", hash_index, ori_mac,h_dest);
					cnt++;
				}
			}
		}
	...
	pr_info("foe_entry_delete_by_mac[count: %d,del_mac:%pM]\n", cnt, del_mac);
}
#endif
...
inline void hnat_set_iif(const struct nf_hook_state *state,
...
static unsigned int is_ppe_support_type(struct sk_buff *skb)
{
	...
	/* if mac header was not set, eth_hdr() = skb->head + 65535 */
	if (!skb_mac_header_was_set(skb))
	{
		return 0;
	}
	...
	case ETH_P_IP:
		...
		/*
		 * some multicast packets may be converted to unicast, so they need to be filtered according to IP.
		 */
		...
		if ((iph->protocol == IPPROTO_TCP) ||
			...
			/* do not accelerate l2tp (over ipsec) traffic */
			if (skb->dev != NULL && strstr(skb->dev->name, "l2tp"))
			{
				break;
			}
			/* do not accelerate pptp vpn traffic */
			if (skb->dev != NULL && (strstr(skb->dev->name, "pptp") || strstr(skb->dev->name, "pppdrv")))
			{
				break;
			}
...
{
	if (skb_hnat_magic_tag(skb) != HNAT_MAGIC_TAG)
		return NF_ACCEPT;
...
#if defined(CONFIG_TP_IMAGE) && defined(CONFIG_TP_MTKHNAT_STATS)
	get_ori_info(skb, state->in);
#endif
				  ...
				  struct flow_offload_hw_path *hw_path)
{
	...
	case IPV4_DSLITE:
	case IPV4_MAP_E:
		entry.ipv4_dslite.dmac_hi = swab32(*((u32 *)eth->h_dest));
		entry.ipv4_dslite.dmac_lo = swab16(*((u16 *)&eth->h_dest[4]));
		entry.ipv4_dslite.smac_hi = swab32(*((u32 *)eth->h_source));
		entry.ipv4_dslite.smac_lo =swab16(*((u16 *)&eth->h_source[4]));
		entry.ipv4_dslite.pppoe_id = hw_path->pppoe_sid;
		break;
	case IPV6_6RD:
	case IPV6_5T_ROUTE:
	case IPV6_3T_ROUTE:
				   ...
				   struct flow_offload_hw_path *hw_path)
{
	...
	case IPV4_MAP_E:
		if (hnat_priv->data->mcast &&
		    is_multicast_ether_addr(&eth->h_dest[0])) {
			entry.ipv4_dslite.iblk2.mcast = 1;
			if (hnat_priv->data->version == MTK_HNAT_V3) {
				entry.bfib1.sta = 1;
				entry.ipv4_hnapt.m_timestamp = foe_timestamp(hnat_priv);
			}
		} else {
			entry.ipv4_dslite.iblk2.mcast = 0;
		}

		entry.ipv4_dslite.iblk2.port_ag =
			(hnat_priv->data->version == MTK_HNAT_V4) ? 0x3f : 0xf;
		break;
	case IPV6_6RD:
	case IPV6_5T_ROUTE:
...
#define is_hnat_homesheild_ipv4_tcp_port(port)\
	(80 == (port) || 8080 == (port) || 8081 == (port))

#define is_hnat_homesheild_ipv4_udp_port(port)\
	(5353 == (port) || 1900 == (port) || 67 == (port) || 68 == (port) || 137 == (port) || 546 == (port) || 547 == (port))

static int ban_hnat_homesheild_port(int ip_ver, int udp, const struct tcpudphdr *pports)
{
	if (4 != ip_ver && 6 != ip_ver)
	{
		pr_info("Unknow ip version: %d\n", ip_ver);
		return 0;
	}

	/* TODO: ipv6 homesheild */
	if (6 == ip_ver)
	{
		return 0;
	}
	...
	if (udp)
	{
		if (is_hnat_homesheild_ipv4_udp_port(pports->src) ||
			is_hnat_homesheild_ipv4_udp_port(pports->dst))
		{
			return -1;
		}
	...
	{
		if (is_hnat_homesheild_ipv4_tcp_port(pports->src) ||
			is_hnat_homesheild_ipv4_tcp_port(pports->dst))
		{
			return -1;
		}
	}
				     ...
				     struct flow_offload_hw_path *hw_path)
{
	...
	case ETH_P_IP:
		...
		case IPPROTO_TCP:
			...
			if (entry.ipv4_hnapt.bfib1.pkt_type == IPV4_DSLITE ||
...
#if defined(CONFIG_TP_IMAGE)
					if (ban_hnat_homesheild_port(4, udp, pptr) < 0)
					{
						return -1;
					}
#endif
				...
				if (skb_vlan_tag_present(skb) && FROM_GE_WAN(skb) && IS_LAN(dev)) {
			...
			if (IS_IPV4_HNAPT(foe)) {
				pptr = skb_header_pointer(skb, iph->ihl * 4,
							  ...
							  &_ports);
#if defined(CONFIG_TP_IMAGE)
				/*l2tp dont't support hwnat right now, just skip */
				if(entry.ipv4_hnapt.sport == 1701 || entry.ipv4_hnapt.dport == 1701)
				{ 
					return -1;
				}
	...
	case ETH_P_IPV6:
...
#if defined(CONFIG_TP_IMAGE)
				_ports.src = foe->ipv6_5t_route.sport;
				_ports.dst = foe->ipv6_5t_route.dport;

				pptr = &_ports;

				if (ban_hnat_homesheild_port(6, udp, pptr) < 0)
				{
					return -1;
				}
		...
		case NEXTHDR_IPIP:
			...
			if ((!mape_toggle &&
...
#if defined(CONFIG_TP_IMAGE)
				_ports.src = foe->ipv4_dslite.sport;
				_ports.dst = foe->ipv4_dslite.dport;
				...
				if (IS_HQOS_MODE) {
#if defined(CONFIG_TP_IMAGE)
					entry.ipv4_hnapt.iblk2.qid = hnat_qos_get_qid(skb);
#else
...
#if defined(CONFIG_TP_IMAGE)
				_ports.src = foe->ipv4_hnapt.new_sport;
				_ports.dst = foe->ipv4_hnapt.new_dport;
...
#if defined(CONFIG_TP_IMAGE)
				_ports.src = foe->ipv6_6rd.sport;
				_ports.dst = foe->ipv6_6rd.dport;
...
#if defined(CONFIG_NET_MEDIATEK_HW_QOS) && defined(CONFIG_TP_IMAGE)
	if (IS_HQOS_MODE)
		qid = hnat_qos_get_qid(skb);
	...
	if (IS_IPV4_GRP(foe)) {
		...
		if (qos_toggle) {
			...
			if (FROM_EXT(skb) || (IS_PPPQ_MODE && !IS_DSA_LAN(dev) && !IS_DSA_WAN(dev)))
...
#if defined(CONFIG_TP_MTKHNAT_STATS)
		memset(&tp_collect_acct[skb_hnat_ppe(skb)][skb_hnat_entry(skb)],
		       0, sizeof(struct mib_entry));
#endif
	}
...
#if defined(CONFIG_NET_MEDIATEK_HW_QOS)
			if (IS_HQOS_MODE && (FROM_GE_LAN(skb) || FROM_GE_WAN(skb) || FROM_GE_VIRTUAL(skb))) {
				bfib1_tx.vpm = 0;
				bfib1_tx.vlan_layer = 1;
				entry->ipv4_hnapt.etype = htons(HQOS_MAGIC_TAG);
				entry->ipv4_hnapt.vlan1 = skb_hnat_entry(skb);
...
static unsigned int mtk_hnat_accel_ct_mark(struct sk_buff *skb)
{
	struct nf_conn *ct;
	enum ip_conntrack_info ctinfo;
	...
	if (unlikely(ct->mark && 
		(NULL != ct_mark_match(&hnat_priv->ct_skip, ct->mark)) ))
	{
		return 0;
	}
...
unsigned int mtk_hnat_nf_post_routing(
	...
	const char *func)
{
...
#if defined(CONFIG_TP_MTKHNAT_CT_SKIP) && defined(CONFIG_TP_IMAGE)
		if (unlikely(!mtk_hnat_accel_ct_mark(skb)))
		{
			break;
		}
#endif
	...
	case HIT_BIND_MULTICAST_TO_GMAC_CPU:
		/*multicast is always disable, forward through CPU*/
...
EXPORT_SYMBOL(mtk_hnat_nf_post_routing);
			   ...
			   const struct nf_hook_state *state)
{
	...
	if (unlikely(skb_headroom(skb) < sizeof(struct hnat_desc))) {
		new_skb = skb_realloc_headroom(skb, sizeof(struct hnat_desc));
		if (!new_skb) {
