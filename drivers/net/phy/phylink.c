...
extern int phy_link_up_flag;
extern int wan_fc_status;
extern int wan_fc_port;
extern int lan_fc_status;
extern int is_1g_wan;
extern int gmac_fc_status;
extern int gmac_id;
#endif
				 ...
				 struct phylink_link_state *state)
{
	...
	if (pl->netdev)
	{
		if(!strncmp(pl->netdev->name, "eth", 3)){
			snprintf(buf, sizeof(buf), "%s%d", "eth", gmac_id);
			if(!strncmp(pl->netdev->name, buf, 4)){
				fc_status_tmp = gmac_fc_status;
			}
		}

		if(phy_link_up_flag != 0 && !strncmp(pl->netdev->name, "lan", 3)){
			snprintf(buf, sizeof(buf), "%s%d", "lan", wan_fc_port);
			if(is_1g_wan != 0 && !strncmp(pl->netdev->name, buf, 4)){
				fc_status_tmp = wan_fc_status;
			}
			else{
				fc_status_tmp = lan_fc_status;
			}
		}

		switch(fc_status_tmp){
			/*Negotiate with the link partner*/
			case 0:
				break;
			/*7531 disable tx flow control, enable rx flow control.
			GMAC enable tx flow control, disable rx flow control.*/
			case 1:
				new_pause = MLO_PAUSE_TX;
				break;
			/*7531 enable tx flow control, disable rx flow control.
			GMAC disable tx flow control, enable rx flow control.*/			
			case 2:
				new_pause = MLO_PAUSE_RX;
				break;
			/*disable trx flow control*/
			case 3:
				new_pause = 0;
				break;
			/*enable trx flow control*/
			case 4:
				new_pause = MLO_PAUSE_TXRX_MASK;
				break;
			default:
				break;
		}
